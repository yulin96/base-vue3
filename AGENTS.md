...
- 桌面端预览由 `src/plugins/setRem.ts` 自动处理：会把 `#app` 按移动端比例缩放、居中展示，并在足够宽时生成二维码。
...
- 优先复用现有能力，不重复造轮子：
  - 页面路由放在 `src/pages`
  - 通用组件优先放/查 `src/components`
  - hooks 优先放/查 `src/hooks`
  - 工具函数优先放/查 `src/utils`
...
- `w-*` 和 `h-*` 这类尺寸写法不要带 `px` 形式。
- 统一使用简化版尺寸类，例如：`w-100 = w-[100px]`、`h-100 = h-[100px]`。
- 需要写固定宽高时，优先写 `w-100`、`h-80` 这种简化形式，不写 `w-[100px]`、`h-[80px]`。

## 请求与业务代码

- 请求统一走 `src/utils/request.ts`。
- 防重复提交优先用 `src/hooks/useLockRequest.ts`。
- 新增接口时优先放到 `src/api`，并补对应类型。

## 构建与发布

- 普通构建使用 `pnpm build` 或 `pnpm build:only`。
- 正式上传使用 `pnpm build:deploy` 或 `pnpm deploy:prod`。
- 测试上传使用 `pnpm build:deploy:test` 或 `pnpm deploy:test`。
