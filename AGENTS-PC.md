...
- 大屏互动项目默认提供统计面板，至少记录开始游玩和完成游玩的数量；其他指标按当前业务流程补充。
- 明确统计口径：没有用户唯一标识时按游玩轮次统计并标注为“人次”；只有能够按用户去重时才标注为“人数”。开始和完成分别在实际进入游玩、达到业务完成条件时计数，同一轮对应事件只记录一次，避免重复点击、重复回调或页面重新挂载导致重复累计。
- 统计同时提供按天明细和历史总计，各指标分别汇总；默认按设备本地日期 `YYYY-MM-DD` 归档，每次事件发生时确定所属日期，保证跨午夜运行时正确分日。
- 统计数据需持久化，刷新页面或重启后保留；优先复用项目已有存储能力，没有服务端统计要求时默认保存在本机，并使用真实项目或活动标识隔离。多设备数据不默认视为已合并统计。
...
- 统计面板默认隐藏，按住 `P` 键展示，松开立即隐藏，不使用按一次切换显隐的方式；窗口失焦或页面进入后台时也要隐藏，避免遗漏 `keyup` 后面板残留。
