...
- `list1`～`list20` 的名称和默认值优先使用 `src/utils/platform/electronConfig.ts` 的 `setupElectronConfig` 集中声明，使用方式见 [Electron 配置说明](src/utils/platform/electronConfig.md)。在项目初始化入口调用，多窗口复用同一份声明；返回值不会自动同步配置面板后续修改。
...
- 新版客户端按完整 `resourceUrl` 隔离 list1～list20 及托管记录，查询参数和 # 内容也参与识别。`saveConfigFile()` 成功后立即强制重启，不可依赖返回后继续业务；普通 `defineConfig()` 默认值初始化不重启。快捷函数需配套支持二十项的新版客户端。
- 配置面板定制优先复用客户端现有接口：
