 ...
 * - defineConfig(patch) 设置托管默认值：字段缺失、仍为初始值或上次托管默认值时更新；用户改为其他值后保留。list1-list20 可用 setupElectronConfig 集中声明。
 * - saveConfigFile() 保存成功后立即强制重启；list1-list20 按完整 resourceUrl 隔离，切换地址时同次请求的 list 值不应用。
